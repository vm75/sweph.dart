...
assets/sweph.wasm: $(SOURCES_CC)
	docker run --rm -v "$(CURDIR)/native/sweph/src:/src" -v "$(CURDIR)/native/utils:/src/utils"  -v "$(CURDIR)/assets:/dist" \
		emscripten/emsdk \
			emcc -o /dist/sweph.wasm $(COMPILER_OPTIONS) $(LINKER_OPTIONS) \
				...
				-D fopen=fOpen -D fclose=fClose -D fread=fRead -D fwrite=fWrite -D rewind=fRewind -D fseek=fSeek -D ftell=fTell -D fgets=fGets -D printf=printF -D getenv=fakeGetenv -D NO_SWE_GLP \
				-s $(COMPILED_EXPORTS) \
				-s FILESYSTEM=0 \
				-s STANDALONE_WASM=1
